...
export class Physics {
  ...
  public syncMeshes(): void {
    this.rigidBodies.forEach((handle) => {
      ...
      handle.mesh.quaternion.set(rotation.x, rotation.y, rotation.z, rotation.w);
    });
  }
  ...
  public createDebugBall(scene: THREE.Scene): THREE.Mesh {
    ...
    const rigidBodyDesc = this.RAPIER.RigidBodyDesc.dynamic().setTranslation(0, 5, 0);
  ...
  public createDebugGround(scene: THREE.Scene): THREE.Mesh {
    ...
    const rigidBodyDesc = this.RAPIER.RigidBodyDesc.fixed().setTranslation(0, -5, 0);
    const rigidBody = this.world.createRigidBody(rigidBodyDesc);
