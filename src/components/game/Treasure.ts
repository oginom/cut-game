...
import type { TreasureConfig, Treasure } from '../../types/game';

export class TreasureFactory {
  /**
   * 宝物の設定を取得
   */
  public static getConfig(type: 'gold' | 'silver' | 'bronze'): TreasureConfig {
    switch (type) {
      case 'gold':
        return {
          type: 'gold',
  ...
  public static create(
    ...
    const colliderDesc = RAPIER.ColliderDesc.cuboid(halfSize, halfSize, halfSize).setDensity(
      config.mass
    );
