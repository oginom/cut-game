...
export class HandTracker {
  ...
  private drawDebug(results: HandLandmarkerResult): void {
    ...
    if (videoAspect > canvasAspect) {
      ...
      scaleX = this.videoElement.videoWidth * (canvas.height / this.videoElement.videoHeight);
      offsetX = (canvas.width - scaleX) / 2;
      offsetY = 0;
    } else {
      // ビデオが縦長：左右がクロップされる
      scaleX = canvas.width;
      scaleY = this.videoElement.videoHeight * (canvas.width / this.videoElement.videoWidth);
      offsetX = 0;
    ...
    if (results.landmarks && results.handedness) {
      for (let i = 0; i < results.landmarks.length; i++) {
        ...
        // ランドマークを描画（座標変換情報を渡す）
        this.drawLandmarks(ctx, landmarks, scaleX, scaleY, offsetX, offsetY, color);
