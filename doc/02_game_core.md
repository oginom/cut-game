...
### 完了条件
- [ ] Three.jsのシーンが正しく初期化される
- [ ] 画面に3Dオブジェクト（テストキューブ）が表示される
- [ ] オブジェクトがアニメーション（回転）している
- [ ] ウィンドウリサイズ時に正しく追従する
...
### 完了条件
- [ ] ロープが物理演算で正しく動作する
- [ ] ロープのセグメントがジョイントで接続されている
- [ ] ロープの最下部に宝物が吊られている
- [ ] ロープ付き宝物が画面左から右（または右から左）へ移動する
- [ ] ロープが物理的に揺れる
- [ ] 画面外に出たロープが自動的に削除される
- [ ] TypeScriptコンパイルエラーなし
...
- [ ] スコアが画面上に表示される
- [ ] コンボシステムが動作する（短時間に複数切るとボーナス）
- [ ] コンボカウンターが表示される
- [ ] スコア加算時にアニメーションが表示される
- [ ] TypeScriptコンパイルエラーなし
