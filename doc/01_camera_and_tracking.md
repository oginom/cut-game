...
### 完了条件
- [x] カメラ映像の上に手のランドマークが表示される
- [x] 両手を同時に認識できる
- [x] 左手と右手が区別されて表示される（色違いなど）
- [x] 手の骨格（接続線）が表示される
- [x] コンソールに検出された手の情報が出力される
- [x] 手を動かすとランドマークが追従する
...
### 完了条件
- [x] カメラ映像の上に顔のバウンディングボックスが表示される
- [x] 顔の6つのキーポイントが表示される
- [x] 手と顔が同時に検出・表示される
- [x] パフォーマンス統計（FPS、検出率）がコンソールに出力される
- [x] TrackingManagerで両トラッカーを統合管理できる

### 検証方法
1. 正面を向く → 顔のバウンディングボックスとキーポイントが表示される
2. 両手を上げる → 手と顔が同時に表示される
3. 顔を左右に動かす → バウンディングボックスが追従する
4. コンソールでパフォーマンス統計を確認 → FPS、手検出率、顔検出率が表示される
